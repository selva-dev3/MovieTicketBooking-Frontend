...
import { dummyShowsData } from '../../../public/images/assets';
        ...
        <>
            ...
            <div className='overflow-x-auto pb-4'>
                <div className='group flex flex-wrap gap-4 mt-4 w-max'>
                    {nowPlayingMovie.map((movie) => (
                        <div onClick={() => setSelectedMovie(movie.id)} key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40
            ...
            <div className='mt-6'>
                ...
                <div className='inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg'>
                    ...
                    <button onClick={handleDateTimeAdd} className='bg-[#f84565]/80 text-white px-3 py-2 text-sm 
                    cursor-pointer rounded-lg hover:bg-[#f84565]'>Add Time</button>
                </div>
                                ...
                                <div className='flex flex-wrap gap-2 mt-1 text-sm'>
                                    {times.map((time) =>(
                                        <div key={time} className='border border-[#f84565] px-2 py-1 flex items-center rounded'>
                                            <span>{time}</span>
                                            <DeleteIcon onClick={()=>handleRemoveTime(date,time)} width={15} className='ml-2 text-red-500 hover:text-red-700
                                            cursor-pointer'/>
                                        </div>
                                    ))}
                                </div>
